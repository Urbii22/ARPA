...
int main(int argc, char* argv[])
{
    ...
    if (mirango == 0) { // Solo el proceso con rango 0 actúa como padre
        printf("[Máquina %s]> Proceso %d de %d: Iniciando proceso padre.\n", nombre, mirango, tamano);
        fflush(stdout);

        // Número de procesos hijos a lanzar
        ...
        printf("[Máquina %s]> Proceso %d: Se han lanzado %d procesos hijos.\n", nombre, mirango, num_hijos);
        fflush(stdout);
        ...
        for (int i = 0; i < num_hijos; i++) {
            ...
            printf("[Máquina %s]> Proceso %d: Enviado saludo al hijo %d.\n", nombre, mirango, i);
            fflush(stdout);
        }
        ...
        for (int i = 0; i < num_hijos; i++) {
            ...
            printf("[Máquina %s]> Proceso %d: Recibido mensaje de hijo %d: %s\n", nombre, mirango, i, mensaje_recibido);
            fflush(stdout);
        }
        ...
        printf("[Máquina %s]> Proceso %d de %d: No es el proceso padre.\n", nombre, mirango, tamano);
        fflush(stdout);
    }
